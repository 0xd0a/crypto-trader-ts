import { INTERVAL, INTERVAL_IN_SECONDS } from "../BrokerManagers/BrokerManager.types";
    ...
    try {
      l1 = await this.db.Quotes.findFirstOrThrow({
        where: {
          ...
          timeframe: INTERVAL_IN_SECONDS[interval],
          opentime: { equals: time },
        },
      });
      ...
      try {
        var kl = await this.binanceManager.getKlines({
          symbol: ticker,
          interval: interval,
      ...
      const a = kl.map((v) => {
        return {
          ...
          timeframe: INTERVAL_IN_SECONDS[interval],
          isCandle: true,
          opentime: new Date(parseInt(v[0])),
          priceO: parseFloat(v[1]),
          priceH: parseFloat(v[2]),
          priceL: parseFloat(v[3]),
          priceC: parseFloat(v[4]),
          volume: parseFloat(v[5]),
          price: 0,
        };
      }); // new Candle({ opentime:v[0],priceO:v[1],priceH:v[2], priceL:v[3], priceC:v[4], volume:v[5] }) })
      // console.log(a)
      try {
        var n_created = await this.db.Quotes.createMany({
          data: a,
          skipDuplicates: true,
        });
      } catch (error) {
