...
export default class QuotesDB {
    ...
    async getQuote(ticker, time) {
        ...
        var l1=await this.db.Quotes.findFirst({
        ...
        if(!l1) {
            ...
            try {
                var kl=await this.binanceManager.client.klines(ticker, '1m', {startTime: time.getTime()})
            } catch (error) {
                console.log("Error while fetching KLINES")
                return null;
            } 
            console.log("KL ", kl)
            
            const a=kl.data.map((v)=>{return { ticker:ticker,isCandle:true, opentime:new Date(parseInt(v[0])),
            ...
            try {
                ...
                l1=await this.db.Quotes.findFirst({
                    where: {ticker: {equals: ticker}, opentime: {equals: time}  }  
                })
