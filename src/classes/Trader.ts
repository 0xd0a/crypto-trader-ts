...
import { BacktestingBrokerManager } from "./BrokerManagers/BacktestingBrokerManager";
import LiveBrokerManager from "./BrokerManagers/LiveBrokerManager";

export default class Trader {
  // finishResolve
  // traderType: {"LIVE", "HISTORY", "LIVEDRYRUN"}
  constructor(
    private traderType = "LIVE",
    private config: Config,
    private db: PrismaClient,
    private lggr: Logger,
    private stats,
    private strategy,
    private binanceMainClient
  ) {
    ...
    if (this.traderType == "LIVE")
      this.brokerManager = new LiveBrokerManager({
        config: config,
        db: db,
        lggr,
        stats,
      ...
      this.brokerManager = new BacktestingBrokerManager({
        config: config,
        db: db,
        lggr,
        stats,
    ...
    else throw Error("Trader traderType not supported");
    ...
    this.brokerManager.setOnDataHandler(
      this.strategy.onData.bind(this.strategy)
    );
    this.brokerManager.setOnInitDataHandler(
      this.strategy.onInitData.bind(this.strategy)
    );
...
class TraderCollection {
  ...
  static getInstance() {
    if (!this.instance) this.instance = new TraderCollection();
  ...
  async terminate() {
    for (var i = 0; i < this.traders.length; i++) await this.traders[i].close();
