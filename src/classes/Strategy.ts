...
import { INTERVAL } from "./BrokerManagers/BrokerManager.types";
export class Strategy {
  ...
  init() {
    //one should subscribe to the same interval, not combined but have to sync streams
    this.brokerManager.subscribeTicker(
      "ETHUSDT",
      INTERVAL.Minute,
  ...
  onData(data) {
    if (data.e == "kline") {
      // only allow data simultaneously
...
export class Strategy2 {
  ...
  onData(data) {
    this.i++;
    if (data.e == "kline") data = data.k;
    if (!this.reverted_position && data.c < this.buy_price)
      this.brokerManager
        .trade("BUY", "USDT", "BNB", 1, data.c, data.t)
        .then((a) => {
    ...
    if (data.c > this.buy_price * 1.02 && this.reverted_position == true)
      this.brokerManager
        .trade("SELL", "BNB", "USDT", 1, data.c, data.t)
        .then((a) => {
